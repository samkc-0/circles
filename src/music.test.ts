...
describe("getToneNote", () => {
  it("gets all of C major in 4th octave correctly", () => {
    expect(getToneNote()(1)).toBe("C4");
    expect(getToneNote()(2)).toBe("D4");
    expect(getToneNote()(3)).toBe("E4");
    expect(getToneNote()(4)).toBe("F4");
    expect(getToneNote()(5)).toBe("G4");
    expect(getToneNote()(6)).toBe("A4");
    expect(getToneNote()(7)).toBe("B4");
  });
  ...
  it('returns "C#5" for key "B", degree 2 in a major scale', () => {
    ...
    expect(getToneNote(context)(2)).toBe("C#5");
  });
  ...
  it('normalizes flats and returns "F4" for key "Db", degree 3 in a major scale', () => {
    ...
    expect(getToneNote(context)(3)).toBe("F4");
  });
  ...
  it('works for a minor scale: key "A", degree 3 in a minor scale', () => {
    ...
    expect(getToneNote(context)(1)).toBe("A4");
    expect(getToneNote(context)(3)).toBe("C5");
  });

  it("Wraps the degree (e.g. 12 will get degree 4)", () => {
    expect(getToneNote()(8)).toBe("C5");
    expect(getToneNote()(12)).toBe("G5");
  });
  ...
  it("works generally, i.e. this sequence is what you'd expect.", () => {
    ...
    const actual = sequence.map((d) => getToneNote()(d));
    expect(actual).toEqual(["C4", "E4", "G4", "B4", "C5"]);
  });
});
